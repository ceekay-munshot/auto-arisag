...
const LIVE_DATA_URL = "/data/investor_dashboard.json";
const LIVE_REFRESH_URL = "/refresh";
...
async function triggerRefresh() {
  ...
  try {
    ...
    if (isLocalRuntime) {
      const response = await fetch(LIVE_REFRESH_URL, {
        method: "POST",
...
function normalizePayload(payload) {
  ...
  ["PV", "2W", "3W", "CV", "TRACTOR", "CE", "E2W", "E3W", "EPV", "ECV"].forEach((category) => {
...
function syncEvOemTracker() {
  const liveSegments = evOemLiveSegments();
  const fallbackSegments = evOemTrackerDatasets();
  const validSegmentIds = (liveSegments.length ? liveSegments : fallbackSegments).map((item) => item.id);
  if (!validSegmentIds.includes(state.evOemSegment)) {
    state.evOemSegment = validSegmentIds[0] || "E2W";
  }
  if (!["M", "Q", "Y"].includes(state.evOemPeriod)) {
    state.evOemPeriod = "M";
  }
  if (liveSegments.length) {
    const selected = liveSegments.find((item) => item.id === state.evOemSegment) || liveSegments[0];
    const periods = asObject(selected?.table?.periods);
    const availablePeriods = ["M", "Q", "Y"].filter((period) => periods[period]);
    if (availablePeriods.length && !availablePeriods.includes(state.evOemPeriod)) {
      const defaultPeriod = selected?.table?.default_period;
      state.evOemPeriod = periods[defaultPeriod] ? defaultPeriod : availablePeriods[0];
    }
  }
}
...
function sectionDefinitions() {
  const sections = [{ id: "source-visibility", label: "Sources" }];

  if (visibleModule("retail")) {
    sections.push({ id: "retail", label: "Retail" });
  }
  if (dashboardData.modules.registration.available && visibleModule("registration")) {
    sections.push({ id: "registration", label: "Registration" });
  }
  if (visibleModule("wholesale")) {
    sections.push({ id: "wholesale", label: "Wholesale" });
  }
  if (visibleModule("components")) {
    sections.push({ id: "components", label: "Components" });
  }
  sections.push({ id: "insights", label: "Insights" });
  sections.push({ id: "company-map", label: "Companies" });
  return sections;
}
...
function syncActiveSection() {
  ...
  if (!sections.some((item) => item.id === state.activeSection)) {
    state.activeSection = sections[0]?.id || "retail";
  }
}

function topLevelSectionForTarget(targetId) {
  if (!targetId) {
    return "retail";
  }
  if (targetId.startsWith("section-wholesale")) {
    return "wholesale";
  }
  if (targetId.startsWith("section-components") || targetId.startsWith("section-segment-share")) {
    return "components";
  }
  if (targetId.startsWith("section-registration") || targetId.startsWith("section-state-registration")) {
    return "registration";
  }
  if (targetId.startsWith("section-company-map") || targetId === "company-drilldown") {
    return "company-map";
  }
  if (targetId.startsWith("section-insights")) {
    return "insights";
  }
  if (targetId.startsWith("section-source-visibility")) {
...
function renderSectionNavigator() {
  const sections = sectionDefinitions();
  const active = sections.find((item) => item.id === state.activeSection) || sections[0];
  return `
    <section id="section-workspace" class="section section-anchor">
      <div class="panel section-nav">
        <div class="section-nav-head">
          <div>
            <p class="section-kicker">Section Workspace</p>
            <h2>Browse one module at a time</h2>
          </div>
          <p class="section-subtitle">This keeps the page tighter and easier to navigate while the data stays unchanged.</p>
        </div>
        <div class="section-tab-row">
          ${sections.map((section) => `
            <button
              class="button section-tab ${section.id === active?.id ? "active" : ""}"
...
function renderActiveSection() {
  switch (state.activeSection) {
    case "source-visibility":
      return renderSourceVisibility();
    case "registration":
      return renderRegistrationSection();
    case "wholesale":
      return renderWholesaleSection();
    case "components":
...
function setupFilters() {
  document.querySelectorAll("[data-filter]").forEach((node) => {
    node.addEventListener("change", (event) => {
      ...
      state.activeSection = topLevelSectionForTarget(pendingScrollTarget);
      render();
    });
  });
}
...
function setupSectionNavigator() {
  document.querySelectorAll("[data-section-nav]").forEach((node) => {
    node.addEventListener("click", () => {
      state.activeSection = node.dataset.sectionNav;
      pendingScrollTarget = "section-workspace";
      render();
    });
  });
}
...
function setupEvOemTracker() {
  ...
  document.querySelectorAll("[data-ev-oem-period]").forEach((node) => {
    node.addEventListener("click", () => {
      state.evOemPeriod = node.dataset.evOemPeriod;
      render();
    });
  });
}
...
function setupCompanyMapCards() {
  document.querySelectorAll("[data-company-map-card]").forEach((node) => {
    node.addEventListener("click", () => {
      ...
      state.activeSection = "company-map";
...
function evOemLiveSegments() {
  const tables = asObject(dashboardData?.modules?.retail?.latest_oem_tables);
  const definitions = [
    { id: "E2W", label: "EV 2W" },
    { id: "EPV", label: "EV Passenger Vehicles" },
    { id: "E3W", label: "EV 3W" },
    { id: "ECV", label: "EV Commercial Vehicles" },
  ];
  return definitions
    .map((definition) => {
      const table = asObject(tables[definition.id]);
      ...
      return {
        id: definition.id,
        label: table.label || definition.label,
        table,
      };
    })
    .filter(Boolean);
  ...
  return [
    {
      ...
      total_units: 191067,
      prior_total_units: 112628,
      source_name: "EVreporter (Vahan) + RushLane",
      source_url: "https://evreporter.com/indias-electric-vehicle-sales-trend-march-2026/",
      note: "March 2026 high-speed e-2W OEM trend from EVreporter, sourced from Vahan Dashboard (as of Apr 2, 2026). RushLane remains the supplemental market-news source. Low-speed e-2W remains excluded.",
      rows: [
        { oem: "TVS Motor", units: 49335, prior_units: 31884, yoy_units: 30772 },
        { oem: "Bajaj Auto", units: 46272, prior_units: 25513, yoy_units: 35215 },
        { oem: "Ather Energy", units: 35736, prior_units: 20839, yoy_units: 15650 },
        { oem: "Hero MotoCorp (Vida)", units: 21447, prior_units: 12600, yoy_units: 8039 },
        { oem: "Ola Electric", units: 10118, prior_units: 3973, yoy_units: 23634 },
        { oem: "Ampere Vehicles", units: 7973, prior_units: 4727, yoy_units: 5670 },
      ],
    },
    {
      id: "E4W",
      ...
      latest_month: "Mar 2026",
      total_units: 22315,
      prior_total_units: 14712,
      source_name: "EVreporter (Vahan) + RushLane",
      source_url: "https://evreporter.com/indias-electric-vehicle-sales-trend-march-2026/",
      note: "March 2026 electric 4W OEM table from EVreporter, sourced from Vahan Dashboard (as of Apr 2, 2026).",
      rows: [
        { oem: "Tata Motors", units: 8253, prior_units: 5947 },
        { oem: "Mahindra & Mahindra", units: 5244, prior_units: 3174 },
        { oem: "JSW MG Motor India", units: 5141, prior_units: 3556 },
        { oem: "Maruti Suzuki India", units: 949, prior_units: 223 },
        { oem: "VinFast Auto India", units: 691, prior_units: 415 },
        { oem: "Hyundai Motor India", units: 476, prior_units: 329 },
      ],
    },
    {
      id: "E3WG",
      ...
      latest_month: "Mar 2026",
      total_units: 3710,
      prior_total_units: 3193,
      source_name: "EVreporter (Vahan) + RushLane",
      source_url: "https://evreporter.com/indias-electric-vehicle-sales-trend-march-2026/",
      note: "March 2026 electric 3W goods OEM trend from EVreporter, sourced from Vahan Dashboard (as of Apr 2, 2026).",
      rows: [
        { oem: "Mahindra Last Mile Mobility", units: 564, prior_units: 571, yoy_units: 703 },
        { oem: "Bajaj Auto", units: 513, prior_units: 453, yoy_units: 539 },
        { oem: "Omega Seiki", units: 326, prior_units: 377, yoy_units: 238 },
        { oem: "Green Evolve", units: 250, prior_units: 151, yoy_units: 82 },
        { oem: "Atul Auto", units: 240, prior_units: 287, yoy_units: 104 },
        { oem: "Euler Motors", units: 229, prior_units: 201, yoy_units: 342 },
      ],
    },
    {
      id: "EBUS",
      ...
      latest_month: "Mar 2026",
      total_units: 559,
      prior_total_units: 578,
      source_name: "EVreporter (Vahan) + RushLane",
      source_url: "https://evreporter.com/indias-electric-vehicle-sales-trend-march-2026/",
      note: "March 2026 electric bus OEM table from EVreporter, sourced from Vahan Dashboard (as of Apr 2, 2026).",
      rows: [
        { oem: "JBM Electric Vehicles", units: 206, prior_units: 151 },
        { oem: "Switch Mobility", units: 161, prior_units: 280 },
        { oem: "PMI Electro Mobility", units: 123, prior_units: 83 },
        { oem: "Pinnacle Mobility", units: 35, prior_units: 2 },
        { oem: "Aeroeagle Automobiles", units: 22, prior_units: 20 },
        { oem: "Azad India Mobility", units: 10, prior_units: 0 },
      ],
    },
  ];
}

function evGrowthPct(current, base) {
...
function evCompoundGrowth(ratePct, periods) {
  const rateValue = Number(ratePct);
  if (!Number.isFinite(rateValue)) {
    return null;
  }
  return (Math.pow(1 + rateValue / 100, periods) - 1) * 100;
}

function evCagrFromGrowth(growthPct, years) {
  const growthValue = Number(growthPct);
  if (!Number.isFinite(growthValue) || !Number.isFinite(years) || years <= 0) {
    return null;
  }
  const ratio = 1 + growthValue / 100;
  if (ratio <= 0) {
    return null;
  }
...
function evOemPeriodColumns(period) {
  if (period === "Q") {
    return [
      { key: "oem", label: "OEM" },
  ...
  if (period === "Y") {
    return [
      ...
      { key: "cagr_7y_pct", label: "7Y CAGR", type: "pct" },
      { key: "share_pct", label: "Current Market Share", type: "pct" },
    ];
  }
...
function evOemPeriodRows(dataset, period) {
  return dataset.rows.map((row) => {
    const momPct = evGrowthPct(row.units, row.prior_units);
    const yoyPct = evGrowthPct(row.units, row.yoy_units);
    const sharePct = dataset.total_units ? (Number(row.units || 0) / Number(dataset.total_units || 0)) * 100 : null;
    const hasPriorUnits = row.prior_units !== null && row.prior_units !== undefined;
    const priorSharePct = dataset.prior_total_units && hasPriorUnits
      ? (Number(row.prior_units) / Number(dataset.prior_total_units)) * 100
      : null;
    const shareChangePp = sharePct !== null && priorSharePct !== null ? sharePct - priorSharePct : null;
...
function renderEvOemTracker() {
  const liveSegments = evOemLiveSegments();
  if (liveSegments.length) {
    const selectedSegment = liveSegments.find((item) => item.id === state.evOemSegment) || liveSegments[0];
    const selectedTable = asObject(selectedSegment.table);
    const periods = asObject(selectedTable.periods);
    const availablePeriodIds = ["M", "Q", "Y"].filter((periodId) => periods[periodId]);
    const fallbackPeriod = periods[selectedTable.default_period] ? selectedTable.default_period : availablePeriodIds[0];
    const period = periods[state.evOemPeriod] ? state.evOemPeriod : fallbackPeriod;
    const periodPayload = asObject(periods[period]);
    const columns = asArray(periodPayload.columns);
    const rows = asArray(periodPayload.rows).filter((row) => {
      if (state.company === "all") {
        return true;
      }
      return asArray(row.listed_companies).includes(state.company) || row.oem === state.company;
    });
    const visibleColumns = pruneEmptyMetricColumns(columns, rows, ["oem"]);
    const downloadColumns = visibleColumns.map((column) => column.key);
    const sourceMeta = asObject(selectedTable.source_meta);
    const sourceUrl = sourceMeta.url || dashboardData.modules.retail?.source_meta?.url;
    const sourceName = sourceMeta.name || "Parivahan Vahan";
    const latestLabel = periodPayload.period_label || sourceMeta.latest_label || "-";
    const totalUnits = rows.reduce((sum, row) => sum + Number(row.current_units ?? row.units ?? 0), 0);

    registerDownload(
      "ev-oem-tracker",
      `ev_oem_tracker_${slugify(selectedSegment.label)}_${period.toLowerCase()}.csv`,
      downloadColumns,
      rows.map((row) => {
        const out = {};
        downloadColumns.forEach((key) => {
          out[key] = row[key];
        });
        return out;
      }),
    ...
    return `
      <article id="section-ev-oem-tracker" class="table-card section-anchor">
        <div class="table-toolbar">
          <div>
            <p class="small-label">EV OEM tracker</p>
            <h3>${selectedSegment.label} leaderboard (${periodLabel(period)})</h3>
          </div>
          <div class="button-row">
        ...
        <div class="state-explorer-controls">
          <label class="filter-field compact">
            ...
            <select data-ev-oem-segment>
              ${liveSegments.map((item) => `
                <option value="${item.id}" ${item.id === selectedSegment.id ? "selected" : ""}>${item.label}</option>
              `).join("")}
            </select>
          </label>
          <div class="state-segment-switch" role="tablist" aria-label="EV OEM tracker periods">
            ${["M", "Q", "Y"].map((item) => `
              <button
                type="button"
                class="button ${period === item ? "button-primary" : ""}"
                data-ev-oem-period="${item}"
                ${periods[item] ? "" : "disabled"}
              >
                ${item}
              </button>
            `).join("")}
        ...
        <div class="stat-inline state-explorer-stats">
          ...
          <div class="stat-block">
            <span class="small-label">${selectedSegment.label} total (shown)</span>
            <strong>${formatUnits(totalUnits)}</strong>
          </div>
          <div class="stat-block">
        ...
        ${renderTable(`ev-oem-live-${selectedSegment.id}-${period}`, visibleColumns, rows)}
        <p class="table-note">${periodPayload.note || sourceMeta.note || "Official maker-wise EV registrations from Parivahan Vahan. No extrapolated growth math is used."}</p>
      </article>
    `;
  }
  ...
  const period = state.evOemPeriod;
  const columns = evOemPeriodColumns(period);
  const rows = evOemPeriodRows(selected, period);
  const visibleColumns = pruneEmptyMetricColumns(columns, rows, ["oem"]);
  const downloadColumns = visibleColumns.map((column) => column.key);

  registerDownload(
    "ev-oem-tracker",
    `ev_oem_tracker_${slugify(selected.label)}_${period.toLowerCase()}.csv`,
    downloadColumns,
    rows.map((row) => {
      const out = {};
      downloadColumns.forEach((key) => {
  ...
  return `
    <article id="section-ev-oem-tracker" class="table-card section-anchor">
      <div class="table-toolbar">
        <div>
          <p class="small-label">EV OEM tracker</p>
          <h3>${selected.label} leaderboard for ${selected.latest_month} (${periodLabel(period)})</h3>
        </div>
        <div class="button-row">
      ...
      <div class="stat-inline state-explorer-stats">
        ...
        <div class="stat-block">
          <span class="small-label">View</span>
          <strong>${periodLabel(period)}</strong>
        </div>
      </div>
      ${renderTable(`ev-oem-${selected.id}-${period}`, visibleColumns, rows)}
      <p class="table-note">${selected.note} Only directly observed baselines are shown; fields without a true source baseline stay n.m.</p>
    </article>
  `;
}
...
function renderOemTables() {
  ...
  const evOemTable = renderEvOemTracker();
    ...
    <div id="section-oem-tracker" class="section-anchor">
      <div class="panel-header compact">
        <div>
          ...
          <h2>Brand share, rank and unit movement in one stacked flow</h2>
        </div>
        <div class="button-row">
          <p class="section-subtitle">All OEM trackers are placed together in full-width rows for faster comparison.</p>
          ${renderSourceAction(dashboardData.modules.retail?.source_meta?.url)}
        </div>
      </div>
      <div class="panel-grid one oem-table-stack">
      ${evOemTable}
...
function renderOemTable(category, table) {
  const sourceMeta = asObject(table.source_meta);
  const sourceName = sourceMeta.name || dashboardData.modules.retail?.source_meta?.name || "FADA";
  const sourceUrl = sourceMeta.url || sourceMeta.validation_url || dashboardData.modules.retail?.source_meta?.url;

  if (table.mode === "periodized" || table.mode === "vahan_live") {
    const periods = asObject(table.periods);
    const periodId = periods[table.default_period]
      ? table.default_period
      : (["M", "Q", "Y"].find((item) => periods[item]) || Object.keys(periods)[0]);
    const periodPayload = asObject(periods[periodId]);
    const rows = asArray(periodPayload.rows).filter((row) => {
      if (state.company === "all") {
        return true;
      }
    ...
    const visibleColumns = pruneEmptyMetricColumns(asArray(periodPayload.columns), rows, ["oem"]);
    const downloadColumns = visibleColumns.map((column) => column.key);
    const latestLabel = periodPayload.period_label
      || sourceMeta.latest_label
      || (sourceMeta.latest_month ? monthLabel(sourceMeta.latest_month) : "-");
    const totalUnits = rows.reduce((sum, row) => sum + Number(row.current_units ?? row.units ?? 0), 0);
    const key = `oem-${category}-${periodId.toLowerCase()}`;

    registerDownload(
      key,
      `${slugify(table.label)}_${periodId.toLowerCase()}_oem_table.csv`,
      downloadColumns,
      rows.map((row) => {
        const out = {};
        downloadColumns.forEach((columnKey) => {
          out[columnKey] = row[columnKey];
        });
        return out;
      }),
    );
    ...
    return `
      <article class="table-card">
        ...
        <div class="stat-inline state-explorer-stats">
          ...
          <div class="stat-block">
            <span class="small-label">${table.label} total (shown)</span>
            <strong>${formatUnits(totalUnits)}</strong>
          </div>
          <div class="stat-block">
            <span class="small-label">Source</span>
        ...
        ${renderTable(key, visibleColumns, rows)}
        <p class="table-note">${periodPayload.note || sourceMeta.note || `Source: ${sourceName} ${latestLabel}.`}</p>
      </article>
    `;
  }
  ...
  const normalizedRows = rows.map((row) => ({
    ...row,
    units: row.units ?? row.current_units ?? 0,
  }));
  const latestLabel = sourceMeta.latest_label
    || (sourceMeta.latest_month ? monthLabel(sourceMeta.latest_month) : null)
    || (dashboardData.modules.retail?.latest_month ? monthLabel(dashboardData.modules.retail.latest_month) : "-");
  const totalUnits = normalizedRows.reduce((sum, row) => sum + Number(row.units || 0), 0);
  const tableColumns = pruneEmptyMetricColumns(
    [
      { key: "oem", label: "OEM" },
  ...
  registerDownload(
    ...
    [...tableColumns.map((column) => column.key), "listed_companies"],
    normalizedRows.map((row) => {
      const out = {};
      tableColumns.forEach((column) => {
        out[column.key] = row[column.key];
      });
      out.listed_companies = asArray(row.listed_companies).join(" | ");
      return out;
    }),
  );
  ...
  return `
    <article class="table-card">
      ...
      ${renderTable(key, tableColumns, normalizedRows)}
      <p class="table-note">Source: ${sourceName} ${latestLabel}. Current units and share are shown against the matching prior-period comparables published by the source.</p>
    </article>
  `;
}
...
function renderComponentsSection() {
  ...
  const showBeneficiaries = beneficiaries.length > 0;
...
function companyMarketShareRows(company) {
  ...
  return asArray(company.categories)
    .map((category) => {
      ...
      const row = asArray(table.rows).find((item) => item.oem === company.company || asArray(item.listed_companies).includes(company.company));
      if (!row) {
        return null;
      }
      return {
...
function hasMeaningfulColumnValue(value, type) {
  if (type === "int" || type === "pct" || type === "pp") {
    return value !== null && value !== undefined && Number.isFinite(Number(value));
  }
  return value !== null && value !== undefined && `${value}`.trim() !== "";
}

function pruneEmptyMetricColumns(columns, rows, alwaysKeep = []) {
  return columns.filter((column) => {
    if (alwaysKeep.includes(column.key)) {
      return true;
    }
    return rows.some((row) => hasMeaningfulColumnValue(row[column.key], column.type));
  });
}
